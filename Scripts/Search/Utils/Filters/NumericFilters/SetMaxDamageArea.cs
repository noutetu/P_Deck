...
// ----------------------------------------------------------------------
public class SetMaxDamageArea : BaseNumericFilterArea<SetMaxDamageArea.DamageComparisonType>
{
    // ======================================================================
    // Constants / 定数
    // ======================================================================
    ...
    {
        public const int DAMAGE_MIN_VALUE = 0;
        public const int DAMAGE_MAX_VALUE = 200;
    ...
    // selectedComparisonType、selectedDamage、OnFilterChangedは基底クラスで管理されるため削除
    ...
    {
        comparisonToggleMap = new Dictionary<Toggle, DamageComparisonType>();
        
        // null チェックを行ってからマッピングに追加
        AddToggleIfNotNull(lessOrEqualToggle, DamageComparisonType.LessOrEqual);
        AddToggleIfNotNull(equalToggle, DamageComparisonType.Equal);
        AddToggleIfNotNull(greaterOrEqualToggle, DamageComparisonType.GreaterOrEqual);
    }

    private void AddToggleIfNotNull(Toggle toggle, DamageComparisonType comparisonType)
    {
        if (toggle != null)
        {
        ...
        {
            Debug.LogWarning($"Toggle for {comparisonType} is null in SetMaxDamageArea. Check Inspector assignments.");
        }
    }

    protected override void PopulateDropdownOptions()
    {
        Debug.Log($"SetMaxDamageArea.PopulateDropdownOptions called. valueDropdown is null: {valueDropdown == null}");
        
        if (valueDropdown == null) 
        {
            Debug.LogError("valueDropdown is not assigned in SetMaxDamageArea. Make sure it's assigned in the Inspector.");
            return;
        }
        ...
        // ダメージ値のオプションを追加
        for (int damage = Constants.DAMAGE_MIN_VALUE; damage <= Constants.DAMAGE_MAX_VALUE; damage += Constants.DAMAGE_INCREMENT)
        {
        ...
        Debug.Log($"SetMaxDamageArea.PopulateDropdownOptions completed. Options count: {valueDropdown.options.Count}");
    }

    protected override DamageComparisonType GetDefaultComparisonType()
    {
        return DamageComparisonType.None; // デフォルトは比較なし
    }
    ...
    protected override void UpdateComparisonToggleVisualState(Toggle toggle, bool isOn)
    {
        ...
        // トグルの有効/無効状態を管理（ダメージ値が選択されていない場合は無効化）
    ...
    {
        // SetMaxDamageArea固有のToggleGroup設定
    ...
    protected override void OnDropdownValueChanged(int index)
    {
        ...
        // SetMaxDamageArea固有の処理: トグルの有効/無効状態を更新
    ...
    public override void ApplyFilterToModel(SearchModel model)
    {
        ...
        {
            Debug.LogError("SearchModel is not initialized in SetMaxDamageArea.");
            return;
        }

        if (IsFilterEffectivelyActive())
        {
            this.searchModel.SetMaxDamageFilter(selectedValue, selectedComparisonType);
        }
        else
        {
            this.searchModel.SetMaxDamageFilter(GetDefaultNumericValue(), DamageComparisonType.None);
        }
    }
    ...
    public override void ResetFilters()
    {
        ...
        // SetMaxDamageArea固有のリセット処理: トグルを無効化
        SetAllTogglesInteractable(false);
    }
    ...
    {
        Debug.Log($"SetMaxDamageArea.ManuallyInitialize called. valueDropdown is null: {valueDropdown == null}");
    ...
    {
        Debug.Log($"SetMaxDamageArea.CheckDropdownState called");
