...
// ----------------------------------------------------------------------
public class SetRetreatCostArea : BaseNumericFilterArea<SetRetreatCostArea.RetreatComparisonType>
{
    // ======================================================================
    // Constants / 定数
    // ======================================================================
    ...
    {
        public const int RETREAT_MIN_VALUE = 0;
        public const int RETREAT_MAX_VALUE = 4;
        public const int RETREAT_INCREMENT = 1;
    ...
    // selectedComparisonType、selectedRetreatCost、OnFilterChangedは基底クラスで管理されるため削除
    ...
    {
        comparisonToggleMap = new Dictionary<Toggle, RetreatComparisonType>();
        AddToggleIfNotNull(lessOrEqualToggle, RetreatComparisonType.LessOrEqual);
        AddToggleIfNotNull(equalToggle, RetreatComparisonType.Equal);
        AddToggleIfNotNull(greaterOrEqualToggle, RetreatComparisonType.GreaterOrEqual);
    }

    /// <summary>
    /// Toggleがnullでない場合のみDictionaryに追加する
    /// </summary>
    private void AddToggleIfNotNull(Toggle toggle, RetreatComparisonType comparisonType)
    {
        if (toggle != null)
        {
        ...
        {
            Debug.LogWarning($"Toggle for {comparisonType} is null in SetRetreatCostArea. Check Inspector assignments.");
        }
    }
    ...
    protected override void PopulateDropdownOptions()
    {
        ...
        {
            Debug.LogError("valueDropdown is not assigned in SetRetreatCostArea. Make sure it's assigned in the Inspector.");
            return;
        }
        ...
        // コスト値のオプションを追加
        for (int cost = Constants.RETREAT_MIN_VALUE; cost <= Constants.RETREAT_MAX_VALUE; cost += Constants.RETREAT_INCREMENT)
        {
    ...
    protected override RetreatComparisonType GetDefaultComparisonType()
    {
        return RetreatComparisonType.None; // デフォルトは比較なし
    }
    ...
    protected override void UpdateComparisonToggleVisualState(Toggle toggle, bool isOn)
    {
        ...
        // トグルの有効/無効状態を管理（コストが選択されていない場合は無効化）
    ...
    {
        // SetRetreatCostArea固有のToggleGroup設定
    ...
    protected override void OnDropdownValueChanged(int index)
    {
        ...
        // SetRetreatCostArea固有の処理: トグルの有効/無効状態を更新
    ...
    public override void ApplyFilterToModel(SearchModel model)
    {
        ...
        {
            Debug.LogError("SearchModel is not initialized in SetRetreatCostArea.");
            return;
        }

        if (IsFilterEffectivelyActive())
        {
            this.searchModel.SetRetreatCostFilter(selectedValue, selectedComparisonType);
        }
        else
        {
            this.searchModel.SetRetreatCostFilter(GetDefaultNumericValue(), RetreatComparisonType.None);
        }
    }
    ...
    public override void ResetFilters()
    {
        ...
        // SetRetreatCostArea固有のリセット処理: トグルを無効化
        SetAllTogglesInteractable(false);
    }
    ...
    /// <summary>
    /// 手動でドロップダウンオプションを設定するメソッド（テスト用）
    /// </summary>
#if DEVELOPMENT_BUILD || UNITY_EDITOR
    public void ManuallyPopulateDropdown()
    {
        Debug.Log($"SetRetreatCostArea.ManuallyPopulateDropdown called. valueDropdown is null: {valueDropdown == null}");
    ...
    {
        Debug.Log($"SetRetreatCostArea.ManuallyInitialize called. valueDropdown is null: {valueDropdown == null}");
    ...
    {
        Debug.Log($"SetRetreatCostArea.CheckDropdownState called");
